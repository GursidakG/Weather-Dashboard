...
import { formatWeatherstackHour, getForecastDays } from "../utils/formatters";

export default function HourlyChart({ forecast, unit }) {
  const firstDay = getForecastDays(forecast)[0];
  const data =
    firstDay?.hourly?.slice(0, 8).map((entry) => ({
      time: formatWeatherstackHour(entry.time),
      temp: Math.round(entry.temperature),
      feels: Math.round(entry.feelslike ?? entry.heatindex ?? entry.temperature),
    })) || [];
      ...
      <div className="chart-wrap">
        {data.length > 0 ? (
          ...
          <p className="empty-state">
            {forecast?.unavailableReason || "Hourly forecast data is unavailable for this key."}
          </p>
        )}
      </div>
    </section>
